...
- 🔬 **Sprint 2 groundwork STARTED — cross-harness proof (Codex).** Build-vs-use fork closed → BUILD (DECISION #21). Added a canonical `scripts/mcp-server.sh` launcher (PATH-robust; any harness points at it) and registered agent-brain with Codex (`codex mcp add agent-brain -- <abs>/scripts/mcp-server.sh`). Verified: the server is **harness-agnostic** (a raw MCP client retrieved a real node) and **Codex connects + discovers the tools** (its log shows `mcp: agent-brain/retrieve started`). **Not yet closed:** a headless Codex tool-CALL fails at non-interactive auto-approval (Codex sandbox/approval × Claude Code's classifier — tooling, not an agent-brain bug). Final check is a 20-sec **interactive** `codex` retrieve (human clicks Allow) — Sprint-2 scope, non-blocking for Sunday.
